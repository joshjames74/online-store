...
import { generateMockAddress, generateMockBasketItem, generateMockOrder, generateMockOrderItem, generateMockProduct, generateMockProductCategories, generateMockReview, generateMockUser } from "@/tests/generate";
...
async function main() {
  ...
  const mockProducts = Array.from({ length: productCount }, () => generateMockProduct(userIds))
  const products = await prisma.product.createManyAndReturn({ data: mockProducts });
  const productIds = products.map(product => product.id);
  console.log("Created products");

  const mockReviews = Array.from({ length: reviewCount }, () => generateMockReview(productIds, userIds));
  const reviews = await prisma.review.createManyAndReturn({ data: mockReviews });
  const reviewIds = reviews.map(review => review.id);
  console.log("Created reviews");

  const mockAddresses = Array.from({ length: addressCount }, () => generateMockAddress(userIds, countryIds));
  ...
  const productCategories = await prisma.productCategory.createManyAndReturn({ data: mockProductCategories });
  console.log("Created product categories");

  const mockOrders = Array.from({ length: orderCount }, () => generateMockOrder(userIds, addressIds, currencyIds));
  const orders = await prisma.order.createManyAndReturn({ data: mockOrders });
  const orderIds = orders.map(order => order.id);
  console.log("Created orders");

  const mockOrderItems = Array.from({ length: orderItemCount }, () => generateMockOrderItem(orderIds, productIds));
  const orderItems = await prisma.orderItem.createManyAndReturn({ data: mockOrderItems });
  const orderItemIds = orderItems.map(orderItem => orderItem.id);
  console.log("Created order items"); 

  const mockBasketItems = Array.from({ length: basketItemCount }, () => generateMockBasketItem(productIds, userIds));
  const basketItems = await prisma.basketItem.createManyAndReturn({ data: mockBasketItems });
