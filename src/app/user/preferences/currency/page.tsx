...
import { Box, Button, Heading, Select, Stack, Text, useToast, UseToastOptions } from "@chakra-ui/react";
...
export default function Page({ params }: { params: { redirectUrl: string }}): JSX.Element {
    ...
    const handleSubmit = () => {
        const pendingToast = toast({ title: "Loading...", status: "loading", isClosable: true })
        const successToast = { title: "Success", status: "success", duration: 1000 } as Omit<UseToastOptions, 'id'>
        const errorToast = { title: "Error", status: "error", duration: 5000 } as Omit<UseToastOptions, 'id'>

        if (!selectedCurrency) { return };

        putUserCurrencyById(user.id, selectedCurrency).then(res => {
        ...
        <Box w="full" justifyItems="center" marginTop="1em">
            <Stack w="2xl">
                ...
                <Stack>
                    ...
                    onChange={event => setSelectedCurrency(parseInt(event.target.value || ''))}>
                    {currencies.map((currency: Currency, index: number) => {
                        return <option key={index} value={currency.id}>{currency.code}</option>
                    })}
                    </Select>

                    <Box display="flex" gap="1em">
                        <Button bgColor={theme.colors.background.secondary} onClick={handleCancel}>Cancel</Button>
                        <Button bgColor={theme.colors.accent.primary} isDisabled={!!!selectedCurrency} onClick={handleSubmit}>Save changes</Button>
                    </Box>
