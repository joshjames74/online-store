...
import { Box, Button, Heading, HStack, Image, Stack, Text, useMediaQuery } from "@chakra-ui/react";
...
export default function OrderProductCard(product: ResultType<'product', { currency: true }>): JSX.Element {
    ...
    const reviewButton = () => {
        return (
            <Text fontSize="xs" _hover={{ textDecoration: "underline" }} onClick={() => router.push(`/product/${product.id}#reviews`)}>Write a review</Text>
        )
    }
            ...
            <HStack justifyContent="space-between" alignItems="stretch">
                <HStack alignItems="stretch">
                    ...
                    <Stack>
                        ...
                        <Text noOfLines={3} textOverflow="ellipsis">{product.description}</Text>
                    </Stack>
                </HStack>
                {reviewButton()}
            </HStack>
