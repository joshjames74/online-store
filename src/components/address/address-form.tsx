...
import { Box, Breadcrumb, BreadcrumbItem, BreadcrumbLink, Heading, Stack, Input, InputGroup, Select, Button, useToast, HStack } from "@chakra-ui/react";
...
import { getAddressesByUserId, postAddress } from "@/api/request/addressRequest";
...
export default function AddressForm(): JSX.Element {
    ...
    const onSubmit = (event: FormEvent<any>) => {
        ...
        const pendingToast = toast({ title: "Saving address...", status: "loading" });

        postAddress(address).then(res => {
            // reload cached addresses
            getAddressesByUserId(user.id, "reload").then(() => {}).catch(error => console.log(error));
            // display success
            toast.update(pendingToast, { title: "Address saved successfully", status: "success", duration: 1000});
            router.push('/user/addresses')
        }).catch(error => {
            toast.update(pendingToast, { title: "Error saving address", status: "error", duration: 5000 })
            console.log(error);
        })
    };
    ...
    return (
        ...
        <Box minW={theme.sizes.minWidth} w="full" display="flex" justifyContent="center" marginY="1em" padding="1em">
                ...
                <form className={styles.form} onSubmit={(event) => handleFormSubmit(event)}>
                    <Stack gap="1em" minW="max-content" maxW="md">
                        <Stack>
                            ...
                            {errors.countryId && errorMessage(errors.countryId.message?.toString())}
                        </Stack>
                        <Stack>
                        ...
                        <Stack>
                            ...
                            {errors.area_code && errorMessage(errors.area_code.message?.toString())}
                        </Stack>
