...
export async function getBasketItemsByUserId(id: number, cache?: RequestCache): Promise<BasketItemWithProductAndCurrency[]> {
...
export async function getBasketByUserId(id: number, cache?: RequestCache): Promise<Basket> {
    const response = await fetch(`/api/user/${id}/basket`, { 
        method: "GET",
        cache: cache ? cache : "force-cache"
    });
