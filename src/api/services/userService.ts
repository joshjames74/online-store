...
export async function upsertUserByEmail(user: Partial<Omit<Usr, 'user_id'>>): Promise<Usr | void> {
    const updateQuery = { name: user.name, image_url: user.image_url };
    const createQuery = { name: user.name, image_url: user.image_url, email: user.email };
    return upsertOneEntityByField('usr', { email: user.email }, updateQuery, createQuery)
}