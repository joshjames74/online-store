...
export type Model = Product | Usr | Review | Address | Country | Currency | Category;

export function generateMany<M extends Model>(count: number, func: () => M): M[] {
...
export const generateMockReview = (productIds: number[], usrIds: number[]): Review => {
...
export const generateMockAddress = (usrIds: number[], countryIds: number[]): Address => {
...
export const generateMockBasketItemWithProduct = (productIds: number[], usrIds: number[]): BasketItemWithProduct => {
...
export const generateMockBasketFromItems = (items: BasketItemWithProduct[]): Basket => {
...
export const generateMockBasket = (productIds: number[], usrIds: number[]): Basket => {

    const count = faker.number.int({ min: 1, max: 20 });
    const items = Array.from({ length: count }, () => generateMockBasketItemWithProduct(productIds, usrIds));
