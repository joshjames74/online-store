...
describe("GET /api/user/by-email/[email]", () => {
  ...
  beforeAll(async () => {
    ...
    const postedUsers = await prisma.usr.createManyAndReturn({ data: users, include: { currency: true, country: true} });
    user = postedUsers[0];
  });
  ...
  it("should return a 200 for a valid user", async () => {
    ...
    const req = new NextRequest(`http://localhost/api/user/by-email/${user.email}`, { method: "GET" });
  ...
  it("should return a 404 for a user that doesn't exist", async () => {
    ...
    const req = new NextRequest(`http://localhost/api/user/by-email/${email}`, { method: "GET" });
