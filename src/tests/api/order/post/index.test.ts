...
describe("POST /api/order", () => {
  ...
  beforeAll(async () => {
    ...
    const countries = await prisma.country.createManyAndReturn({
      data: mockCountries,
    });
    ...
    // create addresses
    const mockAddresses = Array.from({ length: count }, () =>
    ...
    // create basket items
    const mockBasketItems = Array.from({ length: count }, () =>
      generateMockBasketItem(productIds, [activeUser.id]),
    );
    basketItems = await prisma.basketItem.createManyAndReturn({
  ...
  it("valid data: should return the order", async () => {
    ...
    expect(normaliseDate(json.data)).toEqual({
      ...normaliseDate(mockOrder),
      id: json.data.id,
    });
  });
  ...
  it("valid data: should delete the basket items", async () => {
    ...
    const newBasketItems = await prisma.basketItem.findMany({
      where: { usrId: activeUser.id },
    });
