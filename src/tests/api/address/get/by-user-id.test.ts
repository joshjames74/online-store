...
describe("GET /api/user/[id]/addresses", () => {
  ...
  beforeAll(async () => {
    ...
    const testCountries = await prisma.country.createManyAndReturn({
      data: mockCountries,
    });
    ...
    const mockAddresses = Array.from({ length: count }, () =>
      generateMockAddress([activeUser.id], countryIds),
    );
    testAddresses = await prisma.address.createManyAndReturn({
      data: mockAddresses,
  ...
  it("should return a 200 if the user exists", async () => {
    const req = new NextRequest(
      `http://localhost/api/user/${activeUser.id}/addresses`,
      { method: "GET" },
    );
  ...
  it("should return no addresses if the user does not exist", async () => {
    const userId = "1";
    const req = new NextRequest(
      `http://localhost/api/user/${userId}/addresses`,
      { method: "GET" },
    );
