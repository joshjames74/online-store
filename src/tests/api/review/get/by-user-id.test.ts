...
describe("GET /api/user/[id]/reviews", () => {
  ...
  beforeAll(async () => {
    ...
    const mockReviews = Array.from({ length: count }, () =>
      generateMockReview(productIds, [activeUser.id]),
    );
  ...
  it("should return a 200 if the user exists", async () => {
    const req = new NextRequest(
      `http://localhost/api/user/${activeUser.id}/reviews`,
      { method: "GET" },
    );
  ...
  it("should return no reviews if the user does not exist", async () => {
    ...
    const req = new NextRequest(`http://localhost/api/user/${userId}/reviews`, {
      method: "GET",
    });
