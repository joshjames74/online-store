...
import { generateMany, generateMockProduct, generateMockUser } from "@/tests/generate";
...
describe('GET /api/product', () => {
    ...
    beforeAll(async () => {
        ...
        const mockProducts = Array.from({ length: count }, () => generateMockProduct(ids));
        testProducts = await prisma.product.createManyAndReturn({ data: mockProducts, include: { seller: true } });
    });
    ...
    it('no args: should return a 200', async () => {
        const req = new NextRequest(`http://localhost/api/product`, { method: "GET" });
    ...
    it('no args: should include the correct max price', async () => {
        ...
        expect(res.status).toBe(200);
        expect(json.metadata.price.max).toEqual(Math.max(...testProducts.map(product => product.price)));
    })

    // query tests

    it('query: should return all products if an empty query is passed', async () => {
        const req = new NextRequest(`http://localhost/api/product?query=`, { method: "GET" });
        const res = await GET(req);
        const json = await res.json();
    ...
    it('query: should return no products if the query does not match a product title or description', async () => {
        ...
        while (doesMatch) {
            ...
            for (const product of testProducts) {
                if (product.title.toLowerCase().includes(randomString) || product.description.toLowerCase().includes(randomString)) {
                    doesMatch = true;
                    break;
                };
            };
        ...
        const req = new NextRequest(`http://localhost/api/product?query=${randomString}`, { method: "GET" });
        const res = await GET(req);
        const json = await res.json();
    ...
    it('max price: should return no products if max price exceeds min price of products', async () => {
        ...
        const minPrice = Math.min(...testProducts.map(product => product.price)) - 1;

        const req = new NextRequest(`http://localhost/api/product?max_price=${minPrice}`, { method: "GET" });
        const res = await GET(req);
        const json = await res.json();
    ...
    it('max price: should return all products if max price is gte the max price of the products', async () => {
        ...
        const req = new NextRequest(`http://localhost/api/product?max_price=${maxPrice}`, { method: "GET" });
        const res = await GET(req);
        const json = await res.json();
    ...
    it('min review: should return no products if min review exceeds max review score of products', async () => {
        ...
        const minReview = Math.max(...testProducts.map(product => product.review_score)) + 1;

        const req = new NextRequest(`http://localhost/api/product?min_review=${minReview}`, { method: "GET" });
        const res = await GET(req);
        const json = await res.json();
    ...
    it('min review: should return all products if min review is lte the min review of the products', async () => {
        ...
        const minReview = Math.min(...testProducts.map(product => product.review_score));
    ...
    it('per page: should return all products if per page gte the number of products', async () => {
        ...
        const req = new NextRequest(`http://localhost/api/product?perPage=${perPage}`, { method: "GET" });
        const res = await GET(req);
        const json = await res.json();
