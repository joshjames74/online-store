...
import { generateMany, generateMockCountries, generateMockProduct, generateMockProducts, generateMockUser } from "../../../generate";
...
describe('GET /api/product/[id]', () => {
    ...
    it('should return a 200 if the product exists', async () => {
        ...
        const req = new NextRequest(`http://localhost/api/product/${product}`, { method: "GET" });
    ...
    it('should return a 404 if the product does not exist', async () => {
        const req = new NextRequest('http://localhost/api/product/0', { method: "GET" });
    ...
    it('should return a 400 for invalid product id', async () => {
        const req = new NextRequest('http://localhost/api/product/abc£', { method: "GET" });
    ...
    it('should return the correct product data', async () => {
        const product = testProducts[0];
        const req = new NextRequest(`ttp://localhost/api/product/${product.id}`, { method: "GET" });
        const res = await GET(req, { params : { id: product.id.toString() }});
